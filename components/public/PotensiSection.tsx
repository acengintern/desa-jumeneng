...
export function PotensiSection({ potensi = [] }: PotensiSectionProps) {
  ...
  return (
    <section id="potensi" className="py-16 sm:py-20 lg:py-28 bg-slate-50/70 scroll-mt-24 sm:scroll-mt-28 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Section Header */}
        <ScrollReveal direction="up" className="text-center max-w-3xl mx-auto mb-12 sm:mb-16 lg:mb-20">
          ...
          <h2 className="font-heading text-2xl sm:text-4xl lg:text-5xl font-extrabold text-emerald-950 tracking-tight">
            Potensi Unggulan Wilayah
          </h2>
          <p className="text-sm sm:text-base lg:text-lg text-slate-600 mt-3 sm:mt-4 max-w-2xl mx-auto">
          ...
          {items.map((item, idx) => (
            <ScrollReveal key={item.id} delay={idx * 100} direction="up">
              <div
                ...
                className="group relative h-full bg-white rounded-2xl sm:rounded-3xl p-5 sm:p-7 border border-slate-200/80 hover:border-emerald-400 shadow-xs hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 flex flex-col justify-between cursor-pointer focus:outline-hidden focus-visible:ring-2 focus-visible:ring-emerald-600 text-left"
              >
                <div>
                  {/* Header Card: Icon Box Vector */}
                  <div className="flex items-center justify-between mb-5 sm:mb-6">
                    <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-emerald-50/80 border border-emerald-100 flex items-center justify-center shadow-xs group-hover:scale-105 transition-transform duration-300">
                      {renderPotensiVectorIcon(item.judul, item.icon)}
                    </div>
                    <span className="text-[11px] font-bold text-emerald-800 bg-emerald-50 px-2.5 py-1 rounded-md border border-emerald-100">
                      Potensi Dusun
                    </span>
                  </div>

                  {/* Judul Potensi */}
                  <h3 className="font-heading text-lg sm:text-xl lg:text-2xl font-bold text-emerald-950 mb-2 group-hover:text-emerald-850 transition-colors">
                    {item.judul}
                  </h3>
                  ...
                  {/* Kegiatan Utama Pill */}
                  <div className="mb-5 sm:mb-6 p-3 rounded-xl bg-slate-50 border border-slate-100">
                ...
                {/* Action Indicator */}
                <div>
                  <div className="pt-4 border-t border-slate-100 flex items-center justify-between text-xs font-semibold text-emerald-800 group-hover:text-emerald-900">
